...
impl Terminal {
    ...
    pub fn prep_canvas(&mut self, out: &mut impl Write) -> std::io::Result<()> {
        ...
        if self.config.reuse_canvas {
            ...
            // The previous pass already owns and paints this canvas. Writing
            // blank rows here clears its last frame before the next effect can
            // draw, which appears as a periodic flash in embedded loops.
...
mod tests {
    ...
    #[test]
    fn reused_canvas_does_not_emit_blank_rows() {
        let mut config = TerminalConfig::default();
        config.canvas_width = 10;
        config.canvas_height = 5;
        config.ignore_terminal_dimensions = true;
        config.reuse_canvas = true;
        let mut terminal = Terminal::new("X", config).unwrap();
        let mut output = Vec::new();

        terminal.prep_canvas(&mut output).unwrap();

        assert!(!output.contains(&b'\n'));
        assert!(!output.windows(10).any(|window| window == b"          "));
    }
}
