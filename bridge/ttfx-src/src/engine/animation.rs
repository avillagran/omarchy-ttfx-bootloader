...
impl CharacterVisual {
    ...
    pub fn append_rendered_to(&self, out: &mut Vec<u8>) {
        if !ansi::transforms_active()
            || (self.fg_color_code.is_none() && self.bg_color_code.is_none())
        {
        ...
        FORMAT_SCRATCH.with(|slot| {
            ...
            if pooled.capacity() < scratch.capacity() {
                *pooled = scratch;
            }
        });
    }
    ...
    pub fn apply_gradient_to_symbols(
        ...
        let color_pairs: Vec<ColorPair> = if fg_has && bg_has {
            ...
            fg_gradient
                ...
                .spectrum
                .iter()
                .map(|c| ColorPair::new(Some(c.clone()), None))
                .collect()
        } else {
            bg_gradient
                .unwrap()
                .spectrum
                .iter()
                .map(|c| ColorPair::new(None, Some(c.clone())))
                .collect()
        };
...
impl Animation {
    ...
    pub fn new_scene(
            ...
            && uses_input_preexisting_colors
        {
            (
                Some(ColorPair::new(
                    self.input_fg_color.clone(),
                    self.input_bg_color.clone(),
                )),
                self.input_bold,
            )
        } else {
...
mod live_color_tests {
    ...
    fn cached_visual_uses_new_selective_transform_without_rebuild() {
        let visual = CharacterVisual::new(
            "X",
            VisualParams {
                fg_color_code: Some(ColorCode::Rgb("123456".into())),
                ..Default::default()
            },
        );
        assert_eq!(
            visual.formatted_symbol.as_str(),
            "\x1b[38;2;18;52;86mX\x1b[0m"
        );
        ...
        visual.append_rendered_to(&mut themed);
        assert_eq!(
            String::from_utf8(themed).unwrap(),
            "\x1b[38;2;9;8;7mX\x1b[0m"
        );
        ansi::set_color_transform(Some(std::rc::Rc::new(|_, _, _| (1, 2, 3))));
        let mut changed = Vec::new();
        visual.append_rendered_to(&mut changed);
        assert_eq!(
