...
pub fn run_effect(
    ...
    let result = (|| -> std::io::Result<()> {
        ...
        loop {
            ...
            if !ctx.terminal.config.hold_last_frame || frame_has_visible_glyph(&frame) {
...
/// ANSI controls and whitespace do not constitute a visible frame.
fn frame_has_visible_glyph(frame: &str) -> bool {
    #[derive(Clone, Copy)]
    enum State {
        Text,
        Escape,
    ...
    for ch in frame.chars() {
        match state {
            State::Text if ch == '\u{1b}' => state = State::Escape,
            State::Text if !ch.is_whitespace() => return true,
            State::Text => {}
            State::Escape if ch == '[' => state = State::Csi,
            State::Escape => state = State::Text,
            State::Csi if ('@'..='~').contains(&ch) => state = State::Text,
            State::Csi => {}
        }
    }
    false
...
pub fn dump_effect(
    ...
    while let Some(frame) = effect.next_frame(ctx) {
        let duplicate = paced && previous_frame.as_deref() == Some(frame.as_str());
        if !duplicate {
            ...
            if paced {
                ...
                ctx.terminal.enforce_framerate();
...
mod tests {
    ...
    #[test]
    fn visible_frame_filter_ignores_ansi_and_keeps_glyphs() {
        assert!(!frame_has_visible_glyph("\x1b[2J\x1b[1;1H   \n  "));
        assert!(frame_has_visible_glyph("\x1b[38;2;0;255;255m█\x1b[0m"));
    }
