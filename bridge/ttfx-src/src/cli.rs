...
pub struct Cli {
    ...
    /// Preserve the last visible frame while a looped embed starts its next pass.
    #[arg(long = "hold-last-frame", default_value_t = false)]
    pub hold_last_frame: bool,

    /// Render the final text with Omarchy's five horizontal gradient bands.
    #[arg(long = "final-text-bands", default_value_t = false)]
    ...
    /// Embedded renderer: pace and deduplicate framed output instead of
    /// producing the entire animation as a CPU and memory burst.
    #[arg(long = "pace-dump", default_value_t = false, hide = true)]
    pub pace_dump: bool,
